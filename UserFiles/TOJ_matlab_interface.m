...
if experimentType~=0
    warning('You are running the train duration Matlab script, but TD experiment is not selected on TDT, switching experiment type to TD.')
    DA.SetTargetVal('RZ5D.expType',0);
else
    disp('Train duration script')
end
...
if strcmp(respInitialize, 'Yes')
    tank = DA.GetTankName;
    noNum = DA.GetTargetVal('RZ5D.upNum'); % number of no's needed to increase TD
    yesNum = DA.GetTargetVal('RZ5D.downNum'); % number of yes's needed to decrease TD
    ...
    % Since this code is changing the train duration, use the starting
    % pulse amplitude to set the PulseAmp which will be used throughout
    % experiment:
    PulseAmp = DA.GetTargetVal('RZ5D.startingPulseAmp');
    DA.SetTargetVal('RZ5D.PulseAmp', PulseAmp);
    
    % Read in a few more stim parameters from the TDT to save later and to
    % use to calculate the minimum TD
    PW = DA.GetTargetVal('RZ5D.cPW(us)');
    PulseFreq = DA.GetTargetVal('RZ5D.PulseFreqCalc');
    % minimum TD should be the length of one biphasic pulse:
    min_TD = (PW*2)/1000; % in ms            + 41; % 41 us is about the time for one more sample (due to
    ...
    % after the 1st trial (this allows catch trials earlier than the pulse
    % amplitude testing script, because we should be starting in a
    % perceptable range for the TD)
    catchTrial = zeros(numTrials/2-1, 2);
    ...
    catchTrial = [zeros(1,2); catchTrial]; % add the one non-catch trial back to the beginning of each staircase
    ...
    TD = zeros(numTrials, 1);
    startingTD = DA.GetTargetVal('RZ5D.startingTD');
    disp(['Starting train duration will be: ', num2str(startingTD), ' ms, with an amplitude of ', num2str(PulseAmp), ' uA'])
    TD(1) = startingTD;
    
    staircase.intensity = zeros(numTrials/2,2);
    staircase.intensity(1,:) = startingTD;
    
    intensity = zeros(numTrials, 1);
    intensity(1) = startingTD;
...
disp(['TD = ', num2str(TD(1))])
% Once the system is armed, set the first TDT TD to the StartingTD value
pause(1) % make sure there's been enough time to get the first value
DA.SetTargetVal('RZ5D.PTD', TD(1));
disp('System armed');
...
vbl = displayYesNoQuestion(window, white, screenXpixels, screenYpixels);
    exitKeyboardLoop = 0;
    while exitKeyboardLoop == 0
        ...
        elseif keyCode(leftKey) % subject indicated 'yes'
            keyboardResp = 1;
            exitKeyboardLoop = 1;
            vbl = displayYesNoQuestion(window, white, screenXpixels, screenYpixels, 'yes');
        elseif keyCode(rightKey) % subject indicated 'no'
            keyboardResp = 0;
            exitKeyboardLoop = 1;
            vbl = displayYesNoQuestion(window, white, screenXpixels, screenYpixels, 'no');
        end
    ...
    if catchTrial(doubleMatPos) == 1
        disp([displayTrial, ', Train duration = ', num2str(TD(i)), ', Catch'])
    else
        disp([displayTrial, ', Train duration = ', num2str(TD(i))])
    end
    
    % Now turn off the ready to answer indicator, since an answer was just
    % given:
    DA.SetTargetVal('RZ5D.readyForAnswerIndicator', 0);
    % and set the Pulse Amp back to the original starting value (in case a
    % catch trial was just run and you need to change it back from 0)
    DA.SetTargetVal('RZ5D.PulseAmp', PulseAmp);
    ...
    if strcmp(resp,'Yes') % yes was pressed, so step the intensity down
        ...
        intensity(i) = DA.GetTargetVal('RZ5D.PTD'); % record the last train duration that was used
        ...
        if row < numTrials/2
            if yesInaRow(whichStair) == yesNum
                staircase.intensity(doubleMatPos+1) = floor(staircase.intensity(doubleMatPos) - stepSizeVec(doubleMatPos+1)); % use floor here, because if TD had been the min TD, then don't want to end up with e.g., 100+min ~= 100.4096
                yesInaRow(whichStair) = 0; % reset the count
            else % intensity doesn't change
                staircase.intensity(doubleMatPos+1) = floor(staircase.intensity(doubleMatPos));
            end
        end
    ...
    elseif strcmp(resp,'No') % no was pressed, so step the intensity up
        ...
        intensity(i) = DA.GetTargetVal('RZ5D.PTD'); % record the last TD that was used
        ...
        if row < numTrials/2
            if noInaRow(whichStair) == noNum
                staircase.intensity(doubleMatPos+1) = floor(staircase.intensity(doubleMatPos) + stepSizeVec(doubleMatPos+1));       
    ...
    % check if the TD we just set is less than the min. TD, if it is,
    % overwrite it with the min_TD:
    if row < numTrials/2 && staircase.intensity(doubleMatPos+1) < min_TD
        staircase.intensity(doubleMatPos+1) = min_TD;
    end
    ...
    if row < numTrials/2 && catchTrial(doubleMatPos+1)==1
        ...
        % catch trials will use a train duration of 0 ms, and more
        % importantly set the pulse amp to 0 (below). Note that only setting the TD
        % to 0, will still allow one pulse to be triggered
        staircase.intensity(doubleMatPos+1) = 0; % Set the next amp in the staircase to 0 if it's going to be a catch trial
%         DA.SetTargetVal('RZ5D.PulseAmp', 0);
    ...
    % Now that new values are set, update the next TD that will be
    % used
    if i<numTrials
        TD(i+1) = staircase.intensity(find(stairOrderMatrix==i+1));
        DA.SetTargetVal('RZ5D.PTD', TD(i+1)); % send new TD to TDT
        if catchTrial(find(stairOrderMatrix==i+1))==1
            % set pulse amp to 0 during catch trials
            DA.SetTargetVal('RZ5D.PulseAmp', 0);
        end
    end
...
%         DA.SetTargetVal('RZ5D.readyForAnswerIndicator', 1); % set the indicator for answering to on, so experimenter knows that stim (or catch) has been delivered and system is waiting for response
    % first check that the TDT system is still in record mode, if not
        ...
        % and then switch back to the fixation cross 
pause(0.5)
vbl = displayFixationCross(window, white, xCenter, yCenter);
        % Next pause for 1.5 seconds (so we're at 2 sec total) for a brief
        % break and then trigger the TDT to deliver another stim train and
        % start this process over again
        pause(1.5);
         DA.SetTargetVal('RZ5D.noIndicator', 0); % set the indicator back to 0
        DA.SetTargetVal('RZ5D.yesIndicator', 0); % set the indicator back to 0
        
        %         disp(['stim button value ', num2str(DA.GetTargetVal('RZ5D.SetTargetVal'))])
        DA.SetTargetVal('RZ5D.StimButton', 1);
        pause(0.001)
        DA.SetTargetVal('RZ5D.StimButton', 0);
%         disp('sent StimButton')
        
        pause(1.5) % pause for 1.2 seconds while TDT delivers the stimulation (1 second for the audio cue pause, and 200 ms for approximate TD)
        
%         disp('set ready light')
        DA.SetTargetVal('RZ5D.readyForAnswerIndicator', 1); % set the indicator for answering to on, so experimenter knows that stim (or catch) has been delivered and system is waiting for response
    end 
end
...
stair1.intensity = staircase.intensity(staircase.intensity(:,1)~=0,1);
stair1.response = staircase.response(staircase.intensity(:,1)~=0,1);
stair2.intensity = staircase.intensity(staircase.intensity(:,2)~=0,2);
...
if unique(id)==1 % subject only said 'yes'
    legend('3-down, 1-up Staircase 1', 'Staircase 1: Yes',...
        '3-down, 1-up Staircase 2', 'Staircase 2: Yes')
elseif unique(id)==0 % subject only said 'no'
    legend('3-down, 1-up Staircase 1', 'Staircase 1: No',...
        '3-down, 1-up Staircase 2', 'Staircase 2: No')
else % subject responded yes and no
